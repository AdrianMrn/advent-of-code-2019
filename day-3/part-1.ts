...
const [pathOne, pathTwo] = input.split('\n').map(path => path.split(','));

const playingBoardLength = 30000;

const playingBoard: Array<Array<string>> = [];
for (var i = 0; i < playingBoardLength; i++) {
    playingBoard.push(new Array(playingBoardLength));
}

function getNewSymbol(symbol: string, currentSymbol: string) {
...
function drawLine(
    ...
    const [startingRow, startingCol] = connectionPoint;

    // X = overlap symbol
    switch (direction) {
        case 'U':
            for (let i = startingRow; i > startingRow - amount; i--) {
                playingBoard[i][startingCol] = getNewSymbol(symbol, playingBoard[i][startingCol]);
            }

            return [startingRow - amount, startingCol];
        case 'R':
            for (let i = startingCol; i < startingCol + amount; i++) {
                playingBoard[startingRow][i] = getNewSymbol(symbol, playingBoard[startingRow][i]);
            }
...
function drawWire(instructions: Array<string>, symbol: string, startingPoint: [number, number]) {
    let connectionPoint = startingPoint;
...
const centralPortLocation: [number, number] = [playingBoardLength / 2, playingBoardLength / 2];
